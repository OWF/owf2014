...
for (var i = 0; i < 308; i++) {
  PHOTOS.push("/static/pictures/2013/" + i + ".jpg");
}
...
function Flipboard () {
    var   o = this
        ...
        , cards = []
        , images = []
        , card_template = _.template($(CARD_TEMPLATE).html().trim())
        , shuffled_photos = []
        , shuffled_cards = []
        , ready = false
        , flipper
    ...
    function get_random_photo () {
        if (!shuffled_photos || !shuffled_photos.length) {
          shuffled_photos = _.shuffle(PHOTOS);
        }
        return shuffled_photos.shift();
    }

    function get_random_card () {
        if (!shuffled_cards || !shuffled_cards.length) {
